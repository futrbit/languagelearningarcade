...
export default function ClassGenerator() {
  ...
  const updateSkillProgressInFirestore = async (userId, course, retries = 3) => {
    try {
      ...
      while (attempt < retries) {
        try {
          ...
          attempt++;
          console.warn(`Firestore write attempt ${attempt} failed:`, error);
          if (attempt === retries) throw error;
          await new Promise(resolve => setTimeout(resolve, 1000)); // Wait 1s before retry
        }
      }
    } catch (error) {
      console.error("Error updating Firestore after retries:", error);
      setError("Failed to sync progress with server. Disable ad blockers for firestore.googleapis.com and try again.");
    }
  };
  ...
  const syncLessonsToFirestore = async (userId, lessons, homework) => {
    try {
      ...
      setError("Failed to sync lessons with server. Disable ad blockers for firestore.googleapis.com and try again.");
    }
  };
