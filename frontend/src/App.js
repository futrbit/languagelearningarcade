...
import { doc, getDoc, setDoc } from "firebase/firestore";import Login from "./components/Login";
...
import About from "./components/About";import WordChainGame from "./components/WordChainGame";
...
import Crossword from "./components/Crossword";function MainApp({ user, setUser }) {
  const navigate = useNavigate();
  const [selectedRoom, setSelectedRoom] = useState(null);
  const [mode, setMode] = useState("arcade");  const [course, setCourse] = useState(() => {
    try {
  ...
  }, [user]);  const vocab = ["cat", "tiger", "rabbit", "tarantula", "ant", "tortoise", "elephant", "tapir"];
  ...
  const speakingCompleted = course.skills?.find(s => s.skill === "Speaking")?.lessons?.filter(l => l.module_lesson && l.completed).length || 0;  const canAccessRoom = (room) => {
    ...
    if (!user) return <p>Please log in to access the app.</p>;if (mode === "course" && selectedRoom && !canAccessRoom(selectedRoom)) {
  ...
  const [error, setError] = useState(null);  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      if (firebaseUser) {
        ...
        setUser(userId);
        localStorage.setItem("currentUser", userId);    try {
...
return () => unsubscribe();  }, []);  if (authLoading) return <div>Loading...</div>;
              ...
              <SetupPage
                onSave={async (formData) => {
                  try {
                    ...
                    if (!userId) throw new Error("No user");                const missing = [];
