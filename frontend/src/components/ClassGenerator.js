...
import "../styles.css";export default function ClassGenerator() {
  ...
  const [remainingCalls, setRemainingCalls] = useState(null);  const API_URL = process.env.NODE_ENV === "production"
    ? "https://languagelearningarcade.onrender.com"
    : "http://127.0.0.1:8000";  const teachers = [
    ...
    fetchRemainingCalls();
  }, [API_URL]); // Fixed: Added API_URL to dependency array  const initializeCourse = () => {
    ...
    const { item, source } = data;if (source === "starting") {
...
checkDragDropAnswers();  };  const handleDragOver = (e) => {
    e.preventDefault();
    ...
    const correctKeeping = dragItems.slice(2, 4);const startingCorrect =
...
return feedback;  };  const generateClass = async () => {
    if (!auth.currentUser) {
    ...
    if (!checkStorageQuota()) return;setLoading(true);
setError(null);
setClassPlan("");
setFeedback("");
setDragItems([]);
...
try {
  ...
  }, [courseData.skills, validSkills]); // Fixed: Added validSkills to dependency array  return (
    <div>
